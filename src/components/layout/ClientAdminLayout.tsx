...
import { useAdminLayoutVM } from '@/viewmodels/ComponentViewModel/layout/adminLayoutViewModel';
...
export default function ClientAdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      >
        <SideNav collapsed={collapsed} setCollapsed={setCollapsed} setSidebarOpen={setSidebarOpen} />
      </div>
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-center lg:justify-between h-16 p-6 bg-white shadow">
          <button className="lg:hidden text-gray-700 text-2xl pr-3" onClick={() => setSidebarOpen(!sidebarOpen)}>
            <FaBars />
          </button>
          <div className="flex justify-between items-center w-full">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-semibold capitalize text-shadow-xs">{pageTitle}</h1>
