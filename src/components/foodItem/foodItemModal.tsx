...
import { FoodItemModalProps } from "@/models/foodItemModel";


const isVideo = (url: string) => /\.(mp4|webm|ogg)$/i.test(url);

const FoodItemModal = forwardRef<HTMLDivElement, FoodItemModalProps>(
  ({ item, onClose }, ref) => {
    ...
    return (
      <div
        className={`fixed inset-0 z-50 flex justify-center items-center p-4 bg-opacity-50 transition-opacity duration-300 ${
        ...
        >
          ...
          <div className="flex gap-3 my-3">
            <div className="flex justify-between font-bold w-1/5">
              Description<div className="pl-3.5">:</div>
            </div>
